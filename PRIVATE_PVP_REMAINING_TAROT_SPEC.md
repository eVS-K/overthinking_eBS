...
### κ The Fool (`the-fool`) / ο The Hermit (`the-hermit`) — 直前の実カードの継承

| 項目 | The Fool | The Hermit |
| --- | --- | --- |
| 参照先 | 自分の直前ラウンドの物理札 | 相手の直前ラウンドの物理札 |
| 基本強さ | 参照札がなければ0 | 同左 |
| 継承するもの | 参照した札の**現在の局面での**強さ、比較規則、勝敗後能力 | 同左 |
| 物理札 | The Foolのまま | The Hermitのまま |

両者は過去の「解決済み数値」を再生しない。参照先のカード定義を、このラウンドの
局面として改めて解決する。たとえば Joker を継承すれば今の相手の強さを写し、
Death・Strength・Chariot も今回の得点・持ち越し・相手札で判定する。生成、
ロック、対象選択、ノイズ等も、継承したカードの通常の能力としてサーバーが解決する。

- 初回ラウンド、Blank、The Fool、The Hermitを参照した場合は、強さ0・能力なし。
  反響が反響を無限に参照する状態は作らない。
- 手札・履歴上の物理カードはThe Fool／The Hermitのまま残る。一方、比較と能力の
  発動だけは継承先のカードとして扱い、サーバーが通常の権限・対象候補・期限・
  秘匿投影を適用する。
- 旧対局を検証するための `echoProfile` 履歴は互換情報として保持するが、今後の
  The Fool／The Hermitの選択結果はそれを参照して強さを固定しない。
...
The Hanged Manは敗北時、The Starは勝敗・引き分けを問わず、そのラウンドで出された後に
発動する。使用者の相手が、相手自身の手札へ追加したい**カード定義**を選ぶ。候補は、開始時に
凍結した共通デッキに含まる、Blank以外の物理札定義の重複なし集合である。これにより、カードカタログ全体から未承認の
札を作ったり、廃止済み札を復活させたりしない。

- The Hanged Manは敗北時に通常の公開札を一枚追加する。
- The Starは出されたラウンドの結果にかかわらず同じ定義の札を一枚追加するが、所有者以外にはノイズとして見せる。
  所有者は正体を知って通常どおり選べる。実際にプレイされた時だけ公開する。
- 相手が操作しなければ、サーバーが凍結済み候補から一つ選ぶ。候補がなければ不発。
- 生成上限に達しても、既存手札・得点を削らず不発として記録する。
- 対局がそのラウンドの結果で終了する場合は、目的選択の待機状態を作らず、終局を優先する。
