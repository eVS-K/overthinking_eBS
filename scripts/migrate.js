...
async function applyMigrations(pool, migrationsDir = path.join(__dirname, '..', 'migrations')) {
  ...
  try {
    ...
    const applied = new Set((await client.query('SELECT id FROM app_schema_migrations')).rows.map((row) => row.id));
    for (const file of listMigrationFiles(migrationsDir)) {
      ...
      try {
        ...
        await client.query('INSERT INTO app_schema_migrations (id) VALUES ($1)', [file]);
